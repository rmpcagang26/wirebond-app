...
def admin_create_activity():
    ...
    cursor = db.execute(
        ...
        "due_date, is_urgent, progress_type, progress_total, progress_unit, created_by) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        (title, customer, device, category, description, priority, due_date,
         is_urgent, progress_type, progress_total, progress_unit, session['user_id'])
    )
...
def admin_edit_activity(activity_id):
    ...
    db.execute("""
        ...
        progress_unit=? WHERE id=?
    """, (title, customer, device, category, description, status, priority,
          due_date, is_urgent, progress_type, progress_total, progress_unit, activity_id))
    db.commit()
...
def admin_create_user():
    ...
    try:
        db.execute(
            "INSERT INTO users (username, password, full_name, role, shift) VALUES (?, ?, ?, ?, ?)",
            (username, generate_password_hash(password), full_name, role, shift)
        )
        db.commit()
...
def admin_edit_user(user_id):
    ...
    db.execute("UPDATE users SET full_name=?, role=?, shift=?, is_active=? WHERE id=?",
               (full_name, role, shift, is_active, user_id))
    if new_password:
...
def get_calendar_month(year, month, db):
...
def admin_calendar():
    ...
    cal_data = get_calendar_month(year, month, db)
    ...
    return render_template('admin_calendar.html', cal_data=cal_data, year=year, month=month,
                           holidays=holidays, overrides=overrides)
